...
Plot_Population_Density <- function(
  ...
  if (two_tails) {
    ...
    tail_y <- c(
      0, dnorm(low_tail, mean = m0, sd = sigma_m), 0,
  ...
  if (null_hypothesis) {
    if (false_alarm & fill) {
      ...
      if (show_alpha_level) {
        if (two_tails) {
          text(
            x = X_min, y = dnorm(m0, m0, sd = sigma_m) / 15,
            label = bquote(alpha / 2 == .(alpha_value)), pos = 2
          )
        } else {
          text(
            x = X_max, y = dnorm(m0, m0, sd = sigma_m) / 15,
    ...
    if (correct_reject & fill) {
      ...
      if (show_alpha_level) {
        text(
          x = m0 - sigma_m * 0.6, y = dnorm(m0, m0, sd = sigma_m) / 3,
  ...
  if (alternative_hypothesis) {
    if (miss & fill) {
      ...
      if (show_alpha_level) {
        text(
          x = X_max, y = dnorm(m1, m1, sd = sigma_m) / 15,
  ...
  if (!is.null(area)) {
    ...
    if (!is.na(p1) & !is.na(p2)) {
      ...
      area_size <- if (p1 == -Inf) {
        ...
        pnorm(p1, m0, sigma_m, lower.tail = FALSE)
      } else {
        pnorm(p2, m0, sigma_m) - pnorm(p1, m0, sigma_m)
      }
  ...
  if (decision) {
    ...
    text(
      x = X_max, y = dnorm(m0, m0, sd = sigma_m) / 1.1,
      label = expression(paste("Reject ", H[0])), pos = 4
    )
    arrows(
      x0 = X_max, y0 = dnorm(m0, m0, sd = sigma_m) / 1.16,
      x1 = X_max + sigma_m, length = 0.1
    )
    if (two_tails) {
      abline(v = X_min, lty = 3)
      text(
        x = X_min, y = dnorm(m0, m0, sd = sigma_m) / 1.1,
        label = expression(paste("Reject ", H[0])), pos = 2
      )
      arrows(
        x0 = X_min, y0 = dnorm(m0, m0, sd = sigma_m) / 1.16,
  ...
  if (arrow) {
    segments(
      ...
      y1 = c(
        rep(dnorm(m0 + sigma_m, m0, sigma_m), 2), dnorm(m0, m0, sigma_m),
        rep(dnorm(m0 + sigma_m, m0, sigma_m), 2)
      ), lty = 5
    )
    arrows(
