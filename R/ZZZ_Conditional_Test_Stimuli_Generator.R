...
########## 6. Combine different elements together ####################################################################################################
    ...
    Image_Add_Boxes <- magick::image_blank(width = 1024, height = 768, color = "lightgray") %>%
        magick::image_composite(eval(parse(text = Image_Information[Condition, "Box_Top_Left"])),     
            offset = Image_Information[Condition, "Box_Offset_Top_Left"]) %>%
        magick::image_composite(eval(parse(text = Image_Information[Condition, "Box_Top_Right"])),    
            offset = Image_Information[Condition, "Box_Offset_Top_Right"]) %>%
        magick::image_composite(eval(parse(text = Image_Information[Condition, "Box_Bottom_Left"])),  
            offset = Image_Information[Condition, "Box_Offset_Bottom_Left"]) %>%
        magick::image_composite(eval(parse(text = Image_Information[Condition, "Box_Bottom_Right"])), 
            offset = Image_Information[Condition, "Box_Offset_Bottom_Right"])
...
Conditional_Full <- function(
    ...
    Conditional_Test_Audio_Generator_Function <- function(i) Conditional_Test_Audio_Generator(Test_Stimuli_Row = i, 
        ...
        Test_Stimuli_File, Mentioned_Object_Expected_Length)
    print("========== Test Audios ==========")
