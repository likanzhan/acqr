...
Conditional_Information_Generator <- function(
  ...
  Other_Elements_Information <- merge(
    Other_Elements_Audio, Other_Elements_Name,
    by = "File_Name", all.y = TRUE
  )
  Other_Elements_Information <- Other_Elements_Information[
    order(Other_Elements_Information$Position,
      Other_Elements_Information$Condition,
  ...
  Test_Images_Information <- Sentential_Components %>%
    ...
    rename(
      Object_Same_Audio = Component_Audio,
    ...
    mutate(Object_Different_Image = {
      set.seed(acqr::Search_Sample_Seed(Object_Same_Image))
      sample(Object_Same_Image)
    }) %>%
    ungroup() %>%
    mutate(Object_Different_Audio = pull(.[
      match(Object_Different_Image, Object_Same_Image),
      "Object_Same_Audio"
    ])) %>%
    mutate(Object_Different_Audio_Length = pull(.[
      match(Object_Different_Image, Object_Same_Image),
  ...
  Test_Stimuli_Full <- Experimental_Manipulation %>%
    ...
    mutate(Familiarize_1_Object_Image = sapply(
      1:nrow(.),
      function(i) paste0(.[i, grepl(paste0(.[i, "Temporal_Order"], "_Image"), colnames(.))], ".png")
    )) %>%
    mutate(Familiarize_1_Object_Audio = sapply(
      1:nrow(.),
      function(i) paste0(.[i, grepl(paste0(.[i, "Temporal_Order"], "_Audio$"), colnames(.))], ".wav")
    )) %>%
    mutate(Familiarize_1_Object_Audio_Length = sapply(
      1:nrow(.),
      function(i) .[i, grepl(paste0(.[i, "Temporal_Order"], "_Audio_Length$"), colnames(.))]
    )) %>%
    mutate(Familiarize_2_Object_Image = sapply(
      1:nrow(.),
      function(i) {
        paste0(.[i, grepl(paste0("^(?=Object)(?!.*", .[i, "Temporal_Order"], ")(?=.*Image)"),
          colnames(.),
          perl = TRUE
        )], ".png")
      }
    )) %>%
    mutate(Familiarize_2_Object_Audio = sapply(
      1:nrow(.),
      function(i) {
        paste0(.[i, grepl(paste0("^(?=Object)(?!.*", .[i, "Temporal_Order"], ")(?=.*Audio$)"),
          colnames(.),
          perl = TRUE
        )], ".wav")
      }
    )) %>%
    mutate(Familiarize_2_Object_Audio_Length = sapply(
      1:nrow(.),
      function(i) {
        .[i, grepl(paste0("^(?=Object)(?!.*", .[i, "Temporal_Order"], ")(?=.*Audio_Length$)"),
          colnames(.),
          perl = TRUE
        )]
      }
    )) %>%
    mutate(Test_Audio = paste0(abbreviate(paste(
      substr(Test_Image, 1, 2),
      Sentential_Connective, Mentioned_Object, Agent_Mood
    ), 6), ".wav")) %>%
    mutate(
      Top_Left = substr(Spatial_Order, 1, 2),
  ...
  Test_Stimuli <- Test_Stimuli_Full %>%
    select(
      Split, Sentential_Connective, Mentioned_Object, Agent_Mood,
  ...
  Full_Information <- list(
    Audio_File_Information = Audio_File_Information,
...
Cnd_Tst_Img_Gnr <- function(
  ...
  Offset_Top_Left_Box_Closed <- paste0(
    "+", Box_Open_Left_Offset_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Closed
  )
  Offset_Top_Right_Box_Closed <- paste0(
    "+", Box_Open_Right_Offset_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Closed
  )
  Offset_Bottom_Left_Box_Closed <- paste0(
    "+", Box_Open_Left_Offset_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Closed
  )
  Offset_Bottom_Right_Box_Closed <- paste0(
    "+", Box_Open_Right_Offset_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Closed
  )

  ########## 3. Define objects offsets #########################################################################
  Offset_Object_Top_Left <- paste0(
    "+", Box_Open_Left_Offset_X + Object_Adjust_X,
    "+", Box_Open_Top_Offset_Y + Object_Adjust_Y
  )
  Offset_Object_Top_Right <- paste0(
    "+", Box_Open_Right_Offset_X + Object_Adjust_X,
    "+", Box_Open_Top_Offset_Y + Object_Adjust_Y
  )
  Offset_Object_Bottom_Left <- paste0(
    "+", Box_Open_Left_Offset_X + Object_Adjust_X,
    "+", Box_Open_Bottom_Offset_Y + Object_Adjust_Y
  )
  Offset_Object_Bottom_Right <- paste0(
    "+", Box_Open_Right_Offset_X + Object_Adjust_X,
    "+", Box_Open_Bottom_Offset_Y + Object_Adjust_Y
  )

  Height_Difference_Open_Cover <- Box_Open_Height - Box_Cover_Height
  Cover_Offset_Top_Left <- paste0(
    "+", Box_Open_Left_Offset_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Cover
  )
  Cover_Offset_Top_Right <- paste0(
    "+", Box_Open_Right_Offset_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Cover
  )
  Cover_Offset_Bottom_Left <- paste0(
    "+", Box_Open_Left_Offset_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Cover
  )
  Cover_Offset_Bottom_Right <- paste0(
    "+", Box_Open_Right_Offset_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Cover
  )

  Height_Difference_Open_Brand <- Box_Cover_Height - Brand_Same_Height
  Brand_Offset_Top_Left <- paste0(
    "+", Box_Open_Left_Offset_X + Brand_Adjust_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2
  )
  Brand_Offset_Top_Right <- paste0(
    "+", Box_Open_Right_Offset_X + Brand_Adjust_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2
  )
  Brand_Offset_Bottom_Left <- paste0(
    "+", Box_Open_Left_Offset_X + Brand_Adjust_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2
  )
  Brand_Offset_Bottom_Right <- paste0(
    "+", Box_Open_Right_Offset_X + Brand_Adjust_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2
  )
  ...
  Annotation_Offset_Top_Left <- paste0(
    "+", Box_Open_Left_Offset_X + Annotation_Adjust_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2 + Annotation_Adjust_Y
  )
  Annotation_Offset_Top_Right <- paste0(
    "+", Box_Open_Right_Offset_X + Annotation_Adjust_X,
    "+", Box_Open_Top_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2 + Annotation_Adjust_Y
  )
  Annotation_Offset_Bottom_Left <- paste0(
    "+", Box_Open_Left_Offset_X + Annotation_Adjust_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2 + Annotation_Adjust_Y
  )
  Annotation_Offset_Bottom_Right <- paste0(
    "+", Box_Open_Right_Offset_X + Annotation_Adjust_X,
    "+", Box_Open_Bottom_Offset_Y + Height_Difference_Open_Cover +
      Height_Difference_Open_Brand / 2 + Annotation_Adjust_Y
  )
  ...
  Image_Information <- data.frame(
    ...
    Box_Offset_Top_Left = c(
      Offset_Top_Left_Box_Closed, Offset_Top_Left_Box_Open,
      Offset_Top_Left_Box_Open, Offset_Top_Left_Box_Open
    ),
    Box_Offset_Top_Right = c(
      Offset_Top_Right_Box_Open, Offset_Top_Right_Box_Open,
      Offset_Top_Right_Box_Closed, Offset_Top_Right_Box_Open
    ),
    Box_Offset_Bottom_Left = c(
      Offset_Bottom_Left_Box_Open, Offset_Bottom_Left_Box_Closed,
      Offset_Bottom_Left_Box_Open, Offset_Bottom_Left_Box_Open
    ),
    Box_Offset_Bottom_Right = c(
      Offset_Bottom_Right_Box_Open, Offset_Bottom_Right_Box_Open,
      Offset_Bottom_Right_Box_Open, Offset_Bottom_Right_Box_Closed
    ),
    Object_Offset_DD = c(
      Offset_Object_Top_Right, Offset_Object_Bottom_Right,
...
Draw_IA_on_Sample_Image <- function(
  ...
  par(mar = c(0, 0, 0, 0))
  plot(
    x = c(-5, 1024 * 1.05), y = c(-5, 768 * 1.03),
...
Conditional_Full <- function(
  ...
  Conditional_Test_Image_Generator_Function <- function(i) {
    Conditional_Test_Image_Generator(
      Test_Image_Row = i, Test_Image_File,
      Root_Directory, Input_Objects_Folder, Input_Boxes_Folder, Output_Test_Image_Folder
    )
  }
  print("========== Test Images ==========")
  ...
  Conditional_Test_Audio_Generator_Function <- function(i) {
    Conditional_Test_Audio_Generator(
